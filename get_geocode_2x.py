...
def loads(book,sheet,tabela,cursor):
	...
	for row in range(1,s.nrows):
		...
		txt = s.cell(row,1).value
		
		values.append(txt)
		#print txt
		url = "https://maps.google.com/maps/api/geocode/json?address="+txt+"&sensor=false&key=zzzzzzzzzzzzzzzzzzzz"
...
def recursivo(cursor, nrmax):
	...
	for row in linhas:
		...
		url = "https://maps.google.com/maps/api/geocode/json?address="+row[1]+"&sensor=false&key=zzzzzzzzzzzzzzzz"
...
def gerafich(cursor):
	...
	with open(args.fx_coordenadas, 'wb') as csvfile:
		...
		for row in linhas:
			...
			escritor.writerow(( row[0],row[1],locale.format('%.15f',row[2]),locale.format('%.15f',row[3]),row[4],row[5]))
